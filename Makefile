...
COQLIBS?= -R ../QArithSternBrocot QArithSternBrocot\
  -R . CoinductiveReals
COQDOCLIBS?=-R ../QArithSternBrocot QArithSternBrocot\
  -R . CoinductiveReals
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/CoinductiveReals && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "CoinductiveReals" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
